...
(function(exports) {
  ...
  var Glosser = function(options) {
     ...
     * Example Rule: {"key":{"x":"@","relation":"preceeds","y":"aqtu","context":"aqtu-nay-wa-n"},"value":2}
     */
    this.generatePrecedenceForceDirectedRulesJsonForD3 = function(rules, pouchname) {
      if (!pouchname) {
        pouchname = this.currentCorpusName;
      }
      if (!rules) {
      ...
      for (var i in rules) {
        ...
        rules[i].key.firstMorpheme = rules[i].key.x;
        delete rules[i].key.x;
        rules[i].key.secondMorpheme = rules[i].key.y;
        delete rules[i].key.y;
        rules[i].key.value = rules[i].value;

        // Put the firstMorpheme and secondMorpheme morphemes into the morpheme nodes
        morphemes[rules[i].key.firstMorpheme] = morphemes[rules[i].key.firstMorpheme] || {
          name: rules[i].key.firstMorpheme,
          contexts: []
        };
        morphemes[rules[i].key.firstMorpheme].contexts.push(rules[i].key.context);
        morphemes[rules[i].key.secondMorpheme] = morphemes[rules[i].key.secondMorpheme] || {
          name: rules[i].key.secondMorpheme,
          contexts: []
        };
        morphemes[rules[i].key.secondMorpheme].contexts.push(rules[i].key.context);

        //To avoid loops
        if (rules[i].key.secondMorpheme.indexOf("@") === -1) {
    ...
    */
     ...
     */
    this.visualizePrecedenceRelationsAsForceDirectedGraph = function(precedenceGraph, divElement, pouchname) {
      ...
      if (!precedenceGraph) {
        precedenceGraph = this.precedenceGraph;
        if (precedenceGraph) {
          if (precedenceGraph.links.length === 0) {
            precedenceGraph = this.generatePrecedenceForceDirectedRulesJsonForD3();
          }
        } else {
          precedenceGraph = this.generatePrecedenceForceDirectedRulesJsonForD3();
        }
      ...
      precedenceGraph.links.forEach(function(link) {
        link.source = precedenceGraph.nodes[link.firstMorpheme] || (precedenceGraph.nodes[link.firstMorpheme] = {
          name: link.firstMorpheme
        });
        link.target = precedenceGraph.nodes[link.secondMorpheme] || (precedenceGraph.nodes[link.secondMorpheme] = {
          name: link.secondMorpheme
        });
      });
