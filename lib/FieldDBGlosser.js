...
(function(exports) {
  ...
  var Glosser = function(options) {
     ...
     * Example Rule: {"key":{"x":"@","relation":"preceeds","y":"aqtu","context":"aqtu-nay-wa-n"},"value":2}
     */
    this.generatePrecedenceForceDirectedRulesJsonForD3 = function(rules, pouchname, dontConnectWordBoundaries) {
      if (!pouchname) {
        pouchname = this.currentCorpusName;
      }
      if (!rules) {
      ...
      var morphemes = {};
      for (var i in rules) {
        if (!rules[i].key.previous || !rules[i].key.subsequent) {
          console.log("skipping ", rules[i].key);
          continue;
        }
        if (!rules[i].key.previous.morphemes || !rules[i].key.subsequent.morphemes) {
          console.log("skipping ", rules[i].key);
          continue;
        }
        /* make the @ more like what a linguist recognizes for word boundaries */
        if (rules[i].key.previous.morphemes === "@") {
          if (dontConnectWordBoundaries) {
            continue;
          }
          rules[i].key.previous.morphemes = "#_";
        }
        if (rules[i].key.subsequent.morphemes === "@") {
          if (dontConnectWordBoundaries) {
            continue;
          }
          rules[i].key.subsequent.morphemes = "_#";
        }
        // only consider immediate precedence
        if (rules[i].key.distance > 1) {
            continue;
        }
        //Add source target and value to the link
        rules[i].key.utteranceContext = rules[i].value;
        rules[i].key.datumid = rules[i].id;

        // Put the previous and subsequent morphemes into the morpheme nodes
        morphemes[rules[i].key.previous.morphemes] = morphemes[rules[i].key.previous.morphemes] || {
          igt: rules[i].key.previous,
          contexts: []
        };
        morphemes[rules[i].key.previous.morphemes].contexts.push(rules[i].key.context);
        morphemes[rules[i].key.subsequent.morphemes] = morphemes[rules[i].key.subsequent.morphemes] || {
          igt: rules[i].key.subsequent,
          contexts: []
        };
        morphemes[rules[i].key.subsequent.morphemes].contexts.push(rules[i].key.context);

        //To avoid loops
        if (rules[i].key.subsequent.morphemes.indexOf("@") === -1) {
          morphemeLinks.push(rules[i].key);
        }
      }
    ...
    */
     ...
     */
    this.visualizePrecedenceRelationsAsForceDirectedGraph = function(rules, divElement, pouchname, lexiconElement, dontConnectOnWordBoundaries) {
      var precedenceGraph;
      ...
      if (rules) {
        precedenceGraph = this.generatePrecedenceForceDirectedRulesJsonForD3(rules, null, dontConnectOnWordBoundaries);
      }
      if (!precedenceGraph) {
        return;
      }
      if (this.precedenceGraph.links.length === 0) {
        return;
      }
      var lexicon;
      ...
      precedenceGraph.links.forEach(function(link) {
        link.source = precedenceGraph.nodes[link.previous.morphemes] || (precedenceGraph.nodes[link.previous.morphemes] = {
          igt: {
            morphemes: link.previous
          }
        });
        link.target = precedenceGraph.nodes[link.subsequent.morphemes] || (precedenceGraph.nodes[link.subsequent.morphemes] = {
          igt: {
            morphemes: link.subsequent
          }
        });
      });
      ...
      */
      ...
      var force = d3.layout.force()
        .nodes(d3.values(precedenceGraph.nodes))
        .links(precedenceGraph.links)
        .size([width, height])
        .linkStrength(0.2)
        .linkDistance(60)
        .charge(-600);
