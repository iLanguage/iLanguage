...
var FastMap = require("collections/dict");
...
(function(exports) {
  ...
  Lexicon.prototype = Object.create(FastMap.prototype, {
    constructor: {
      value: Lexicon
    },
    bindToView: {
      value: function() {
        ...
        for (entryIndex in entries) {
          ...
          entryvalue = entries[entryIndex][1];
  ...
  var LexiconFactory = function(options) {
    var lex = new Lexicon();
    lex.precedenceRelations = new Set();
    if (options.precedenceRelations) {
      for (var i in options.precedenceRelations) {
        ...
        var node = lex.get(options.precedenceRelations[i].key.previous.morphemes);
        if (!node) {
          node = {
            igt: options.precedenceRelations[i].key.previous,
            contexts: new Set()
          };
          lex.add(node, options.precedenceRelations[i].key.previous.morphemes);
        }
        node.contexts.add(options.precedenceRelations[i].key.context)

        var secondnode = lex.get(options.precedenceRelations[i].key.subsequent.morphemes);
        if (!secondnode) {
          secondnode = {
            igt: options.precedenceRelations[i].key.subsequent,
            contexts: new Set()
          };
          lex.add(node, options.precedenceRelations[i].key.subsequent.morphemes);
        }
        secondnode.contexts.add(options.precedenceRelations[i].key.context)
